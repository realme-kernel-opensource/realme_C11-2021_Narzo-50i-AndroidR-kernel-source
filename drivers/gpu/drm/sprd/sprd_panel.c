...
extern int himax_get_black_screentest_status(void);
...
static int sprd_panel_unprepare(struct drm_panel *p)
{
	...
	int items, i;
    int black_test_status = 0;
	DRM_INFO("%s()\n", __func__);
    
    if (strstr(lcd_name, "lcd_ft"))
        black_test_status = get_black_screentest_status();
    else
	    black_test_status = himax_get_black_screentest_status();
		
    printk("lcd black status = %d \n",black_test_status);
	if(!black_test_status){
	   ...
	   DRM_ERROR("now is test tp black screen need lcd power\n");
	}
...
/*add resume_power_on_timimg for lcd&tp timing Compatible*/
static int resume_power_on_timimg(struct sprd_panel *panel_t)
{
    int ret = 0,items = 0;
	struct gpio_timing *timing;
    if (strstr(lcd_name, "lcd_ft8006s"))
    {
      if (panel_t->info.reset_gpio) {
        gpiod_direction_output(panel_t->info.reset_gpio, 0);
      }
	  fts_tp_reset_pin(0);
      mdelay(10);
      ...
      if (panel_t->info.avdd_gpio) {
        gpiod_direction_output(panel_t->info.avdd_gpio, 1);
        mdelay(5);
      }
      
      if (panel_t->info.avee_gpio) {
         gpiod_direction_output(panel_t->info.avee_gpio, 1);
         mdelay(5);
      }
      ...
      mdelay(3);
    ...
    }else if(strstr(lcd_name, "jd9365t_ctc")) { // 如果lcd_name为"lcd_xxxxx"	
		...
		if (panel_t->info.reset_gpio) {
			items = panel_t->info.rst_on_seq.items;
			timing = panel_t->info.rst_on_seq.timing;
			gpiod_direction_output(panel_t->info.reset_gpio,
						timing[0].level);
			mdelay(timing[0].delay);
		}
...
static int sprd_panel_prepare(struct drm_panel *p)
{
	...
	ret = resume_power_on_timimg(panel);

	if(!strcmp(lcd_name, "ft8722")) { // 如果lcd_name为"lcd_xxxxx"
		// 1. rst(0)
		if (panel->info.reset_gpio) {
			items = panel->info.rst_on_seq.items;
			timing = panel->info.rst_on_seq.timing;
			gpiod_direction_output(panel->info.reset_gpio,
						timing[0].level);  // rst(0)
			mdelay(timing[0].delay); // >3ms
		}
	...
	}else if(!strcmp(lcd_name, "icnl9911C")){
		mdelay(1);
		if (panel->info.reset_gpio) {
				items = panel->info.rst_on_seq.items;
				timing = panel->info.rst_on_seq.timing;
	...
	}else if(!strcmp(lcd_name, "ft8615")) { // 如果lcd_name为"lcd_xxxxx"
		// 1. rst(0)
		if (panel->info.reset_gpio) {
			items = panel->info.rst_on_seq.items;
			timing = panel->info.rst_on_seq.timing;
...
static int sprd_oled_set_brightness(struct backlight_device *bdev)
{
	...
	if (oled->cmds_total == 1) {
 	 	if (oled->cmds[0]->wc_l == 3) {
					if(strstr(lcd_name,"ft8006s")){
						...
						oled->cmds[0]->payload[1] = brightness >> 8 & 0xF;
						oled->cmds[0]->payload[2] = brightness & 0xFF;
					}else if(strstr(lcd_name,"icnl9911C")){
						oled->cmds[0]->payload[1] = brightness >> 8 & 0xF;
						oled->cmds[0]->payload[2] = brightness & 0xFF;
					}else if(strstr(lcd_name, "jd9365t_ctc")){
						oled->cmds[0]->payload[1] = brightness >> 8 & 0xF;
						oled->cmds[0]->payload[2] = brightness & 0xFF;
					}		
					else{
	...
	printk("liang:[0X%x] :[0X%x] [0X%x] \n",oled->cmds[0]->payload[0],oled->cmds[0]->payload[1],oled->cmds[0]->payload[2]);
	
	if((0== brightness) && (strstr(lcd_name, "jd9365t_ctc"))){	
	
		oled->cmd_len = 6;
		oled->cmds[0]->data_type =0x39;
		oled->cmds[0]->wait = 0;
		oled->cmds[0]->wc_h = 0;
		oled->cmds[0]->wc_l = 2;
		oled->cmds[0]->payload[0] = 0x53;
		oled->cmds[0]->payload[1] = 0;
		sprd_panel_send_cmds(panel->slave,
				oled->cmds[0],
				oled->cmd_len);
		printk("liang send :[0X%x] :[0X%x] [0X%x] \n",oled->cmds[0]->payload[0],oled->cmds[0]->payload[1],oled->cmds[0]->payload[2]);
		mdelay(50);
	}
