...
static void sprd_sdhc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	if (ios->power_mode != host->ios.power_mode) {
		...
		case MMC_POWER_OFF:
			...
			//CRP:SPCSS00778980
			if ((strcmp(host->device_name, "sdio_sd") == 0)) {
				ret = gpio_direction_output(host->tf_ldo_en, 0);
				dev_err(dev, "SD: gpio direction output tf_ldo_en 0 ret:%d tf_ldo_en:%d\n", ret, host->tf_ldo_en);
				gpio_set_value(host->tf_ldo_en,0);
			}else{
		...
		case MMC_POWER_UP:
			...
			//CRP:SPCSS00778980
			if ((strcmp(host->device_name, "sdio_sd") == 0)) {
				ret = gpio_direction_output(host->tf_ldo_en, 1);
				dev_err(dev, "SD: gpio direction output tf_ldo_en 1 ret:%d tf_ldo_en:%d\n", ret, host->tf_ldo_en);
				gpio_set_value(host->tf_ldo_en,1);
			}else{
				if (!IS_ERR(mmc->supply.vmmc))
					mmc_regulator_set_ocr(host->mmc,
		...
		struct sprd_sdhc_host *host)
{
	...
	dev_err(dev, "SD: of get name cd-gpios:%d\n", host->detect_gpio);
	if (!gpio_is_valid(host->detect_gpio)) {
		host->detect_gpio = -1;
		dev_err(dev, "SD: is valid cd-gpios:%d\n", host->detect_gpio);
	} else {
		sprd_get_fast_hotplug_info(np, host);
		host->detect_gpio_polar = flags;
	}

	host->tf_ldo_en = of_get_named_gpio(np, "tf_ldo_en", 0);
	if (host->tf_ldo_en < 0){
		dev_err(dev, "SD: of get name tf_ldo_en failed ret:%d tf_ldo_en:%d\n", ret ,host->tf_ldo_en);
		host->tf_ldo_en = 221;
	}
	if (!gpio_is_valid(host->tf_ldo_en)) {
		dev_err(dev, "SD: is valid tf_ldo_en:%d\n", host->tf_ldo_en);
		host->tf_ldo_en = -1;
	}else{
			ret = gpio_request(host->tf_ldo_en, "tf_ldo_en");
			dev_err(dev, "SD: gpio requset tf_ldo_en ret:%d tf_ldo_en:%d\n", ret, host->tf_ldo_en);
			if(ret == -EPROBE_DEFER){
					return -EPROBE_DEFER;
			}else{
				ret = gpio_direction_output(host->tf_ldo_en, 1);
				dev_err(dev, "SD: gpio direction output tf_ldo_en ret:%d tf_ldo_en:%d\n", ret, host->tf_ldo_en);
		}
	}
...
static ssize_t show_tf_det_status(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct platform_device *pdev =
        container_of(dev, struct platform_device, dev);
    struct sprd_sdhc_host *host = platform_get_drvdata(pdev);
    ...
    tf_det_status = mmc_gpio_get_cd(host->mmc);
	printk("SD:cat value of tf detect! (tf_det=%d)\n", tf_det_status);
    if (tf_det_status)
...
static DEVICE_ATTR(tf_det_status, 0444, show_tf_det_status, NULL);
...
static struct attribute *tf_det_attributes[] = {
    &dev_attr_tf_det_status.attr,
    NULL
};

static struct attribute_group tf_det_attribute_group = {
    .attrs = tf_det_attributes
};
...
static int sprd_sdhc_probe(struct platform_device *pdev)
{
	...
	if (ret == -EPROBE_DEFER){
		dev_err(dev, "EPROBE_DEFER to get basic resource: %d\n", ret);
		return -EPROBE_DEFER;
	}
	if (ret) {
    ...
    ret = sysfs_create_group(&(pdev->dev.kobj), &tf_det_attribute_group);
    if (ret < 0) {
        printk("SD:create sysfs for tf detect failed! (ret=%d)\n", ret);
        goto err_free_host;
    }
...
static int sprd_sdhc_remove(struct platform_device *pdev)
{
	...
	sysfs_remove_group(&(pdev->dev.kobj), &tf_det_attribute_group);
