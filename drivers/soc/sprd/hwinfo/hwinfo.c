...
#define DEFINE_STR_INFO_SETUP(__match)				\
static int __init __match ## _setup(char *str)			\
{								\
	oplus_ver_info.__match = str;				\
	return 0;						\
}								\
__setup(#__match"=", __match ## _setup)

DEFINE_STR_INFO_SETUP(prj_name);
DEFINE_STR_INFO_SETUP(pcb_version);
DEFINE_STR_INFO_SETUP(boot_mode);
DEFINE_STR_INFO_SETUP(rf_type);
DEFINE_STR_INFO_SETUP(secure_type);
DEFINE_STR_INFO_SETUP(secure_stage);
DEFINE_STR_INFO_SETUP(ocp);
DEFINE_STR_INFO_SETUP(serial_id);
DEFINE_STR_INFO_SETUP(eng_version);
DEFINE_STR_INFO_SETUP(is_confidential);
DEFINE_STR_INFO_SETUP(dtsi_name);
DEFINE_STR_INFO_SETUP(audio_name);
