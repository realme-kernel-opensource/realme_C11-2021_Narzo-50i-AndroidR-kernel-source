...
extern unsigned int notify_code;
extern bool more_than_temp53;
...
static int runin_stop_flag = 0;// 0-->charging   1-->stop charge
...
struct bq2560x_charger_info *pinfo = NULL;
...
static int bq2560x_detect_device(struct bq2560x_charger_info *info)
{
    int ret;
    u8 reg_val;
    ...
    if(ret == 0){
        ...
        chipid_num = part_no;
        dev_info(info->dev, "bq2560x Part Num:%d",part_no);
    }
...
void bq2560x_dump_regs(struct bq2560x_charger_info *info)
{
	...
	for (addr = 0x0; addr <= 0x0B; addr++) {
		ret = bq2560x_read(info,addr,&val);
		if (ret == 0){
...
bq2560x_charger_set_ovp(struct bq2560x_charger_info *info, u32 vol)
{
	...
	dev_err(info->dev, "set bq2560x_charger_set_ovp:ovp:%x\n",reg_val);
	return bq2560x_update_bits(info, BQ2560X_REG_6,
...
static int bq2560x_disable_batfet(struct bq2560x_charger_info *bq)
{
	const u8 val = REG07_BATFET_OFF << REG07_BATFET_DIS_SHIFT;

	return bq2560x_update_bits(bq, BQ2560X_REG_7, REG07_BATFET_DIS_MASK,
				val);
}

int bq2560x_set_ship_mode(struct bq2560x_charger_info *bq)
{
	int ret;
	ret = bq2560x_disable_batfet(bq);
	if (ret){
		dev_warn(bq->dev, " bq2560x set ship mode\n");
		return ret;
	}
...
int bq2560x_disable_safety_timer(struct bq2560x_charger_info *bq)
{
	const u8 val = REG05_CHG_TIMER_DISABLE << REG05_EN_TIMER_SHIFT;

	return bq2560x_update_bits(bq, BQ2560X_REG_5, REG05_EN_TIMER_MASK,
				val);
}
...
static int bq2560x_charger_hw_init(struct bq2560x_charger_info *info)
{
	...
	if (ret) {
		...
		bq2560x_disable_safety_timer(info);
		...
		ret = bq2560x_charger_set_termina_cur(info, 240);
		if (ret) {
...
static void bq2560x_charger_stop_charge(struct bq2560x_charger_info *info)
{
	...
	if (info->role == BQ2560X_ROLE_MASTER_DEFAULT) {
		if (!present || info->need_disable_Q1) {
			...
			dev_err(info->dev, "%s :%d ,enable HIZ mode success\n", __func__, __LINE__);
		}
		...
		dev_err(info->dev, "%s :%d ,disable bq2560x charge success\n", __func__, __LINE__);
	} else if (info->role == BQ2560X_ROLE_SLAVE) {
		ret = bq2560x_update_bits(info, BQ2560X_REG_0,
					  BQ2560X_REG_EN_HIZ_MASK,
					  0x01 << BQ2560X_REG_EN_HIZ_SHIFT);
		if (ret)
			dev_err(info->dev, "enable HIZ mode failed\n");
		dev_err(info->dev, "bq2560x_charger_stop_charge_hzl03\n");
...
out:
	...
	printk("bq2560x_charger_work: battery present = %d, charger type = %d, limit current= %d, current= %d\n",
		 present, info->usb_phy->chg_type,limit_cur,cur);
	if (runin_stop_flag == 0)
				      ...
				      unsigned long limit, void *data)
{
	...
	if (info->role == BQ2560X_ROLE_SLAVE)
		return NOTIFY_OK;
	if (runin_stop_flag == 0){// 0-->charging	 1-->stop charge
...
void charger_dev_disable_charge(struct bq2560x_charger_info *info ,bool enable)
{
	int ret = 0;
	if(enable)
		ret = bq2560x_charger_set_runin_stop(info,0);
	else
		ret = bq2560x_charger_set_runin_stop(info,1);

	if (ret)
	 		dev_err(info->dev, "disable_charge error\n");
	printk("disable charge:%d",enable);
}
...
{
	if (info != NULL)
		return bq2560x_charger_start_charge(info);
