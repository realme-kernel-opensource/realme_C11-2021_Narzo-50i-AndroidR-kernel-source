				   ...
				   int chg_sts)
{
	...
	dev_info(data->dev, "%s, temp 1= %d\n", __func__, temp);
	if (temp > 0)
		temp = temp + data->cur_1000ma_adc / 2;
	else
		temp = temp - data->cur_1000ma_adc / 2;
	dev_info(data->dev, "%s, temp 2= %d, 100ma_adc/2 = %d\n", __func__, temp, data->cur_1000ma_adc / 2);

	temp = div_s64(temp, data->cur_1000ma_adc);
	dev_info(data->dev, "%s, temp 3= %d\n", __func__, temp);
	...
	*cap = delta_cap + data->init_cap;
	...
	dev_info(data->dev, "init_cap = %d, init_clbcnt = %d, cur_clbcnt = %d, normal_cap = %d, "
         "delta_cap = %d, Tbat  = %d, *cap = %d\n",
         data->init_cap, data->init_clbcnt, cur_clbcnt,
         data->normal_temperature_cap, delta_cap, data->bat_temp, *cap);
