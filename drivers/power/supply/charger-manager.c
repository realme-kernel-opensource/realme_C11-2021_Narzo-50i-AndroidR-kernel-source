...
#define CHARGER_HV_THR	5800000

#define ATL_BATTERY_VOLTAGE_MIN_5000  550
#define ATL_BATTERY_VOLTAGE_MAX_5000  790
#define LIWINON_BATTERY_VOLTAGE_MAX_5000    180
#define LIWINON_BATTERY_VOLTAGE_MIN_5000    0
int bat_id = 0;
...
bool more_than_temp53 =false;

static int charger_full_count = 0,rechg_flag = 0;

#define NTC_DISCONNECT_TEMP		-250
static bool temp_stop_charging_flag = false;

static int shutdown_low_volt_count = 0;
static int shutdown_low_cap_count = 0;
static int ui_cap;

#define WARM_FULLBAT_VOLTAGE	4090000
#define HW_BAT_FULL_VOLT 30000

static bool sdp_flag = false;
...
extern struct bq2560x_charger_info *pinfo;
extern void charger_dev_stop_charge(struct bq2560x_charger_info *info);
extern int charger_dev_start_charge(struct bq2560x_charger_info *info);
extern int bq2560x_set_ship_mode(struct bq2560x_charger_info *bq);
extern void bq2560x_dump_regs(struct bq2560x_charger_info *info);
extern int bq2560x_disable_safety_timer(struct bq2560x_charger_info *bq);
extern int bq2560x_charger_set_termina_cur(struct bq2560x_charger_info *info, u32 cur);
extern void charger_dev_disable_charge(struct bq2560x_charger_info *info ,bool enable);

int shipmode = 0;
static int aging_status = 0;
...
static bool is_full_charged(struct charger_manager *cm)
{
	...
	if(sdp_flag==true && chipid_num == 0x09){
		sdp_flag = false;
	   desc->fullbatt_uA = desc->fullbatt_uA+1000;
	}
	...
	if (desc->fullbatt_uV > 0 && desc->fullbatt_uA > 0) {
		...
		printk("%s ,uV:%d fullbatt_uV:%d uA:%d fullbatt_uA:%d",__func__,uV,desc->fullbatt_uV,uA,desc->fullbatt_uA);
		if (uV >= desc->fullbatt_uV && uA <= desc->fullbatt_uA && uA > 0) {
			...
			if (++desc->trigger_cnt > 1) {
				if (cm->desc->cap >= CM_CAP_FULL_PERCENT) {
					...
					printk("%s ,charge full uV:%d fullbatt_uV:%d uA:%d fullbatt_uA:%d",__func__,uV,desc->fullbatt_uV,uA,desc->fullbatt_uA);
					charger_full_count = 0;
					is_full = true;
					charger_dev_stop_charge(pinfo);
					bq2560x_dump_regs(pinfo);
					}
				} else {
					printk("%s ,charge full yet uV:%d fullbatt_uV:%d uA:%d fullbatt_uA:%d",__func__,uV,desc->fullbatt_uV,uA,desc->fullbatt_uA);
					is_full = false;
					adjust_fuel_cap(cm, CM_FORCE_SET_FUEL_CAP_FULL);
				}
			...
			goto out;
		} else if(cur_jeita_status == STATUS_T2_TO_T3 && uV >= WARM_FULLBAT_VOLTAGE && uA <= desc->fullbatt_uA && uA > 0){
			charger_full_count++;
			if(charger_full_count > 3){
				printk("%s ,charge full uV:%d fullbatt_uV:%d uA:%d fullbatt_uA:%d",__func__,uV,WARM_FULLBAT_VOLTAGE,uA,desc->fullbatt_uA);
				charger_full_count = 0;
				charger_dev_stop_charge(pinfo);
			}
			...
			if(hw_full_counts > 3){
					...
					is_full = true;
					charger_dev_stop_charge(pinfo);
					printk("%s ,hw charge full uV:%d fullbatt_uV:%d uA:%d fullbatt_uA:%d",__func__,uV,desc->fullbatt_uV,uA,desc->fullbatt_uA);
				}
			}else{
			charger_full_count = 0;
			hw_full_counts = 0;
...
static int try_charger_enable_by_psy(struct charger_manager *cm, bool enable)
{
	...
	if (timeout_flag == true) {
		enable = POWER_SUPPLY_STATUS_CHARGING;
	}
...
static void fullbatt_vchk(struct work_struct *work)
{
	...
	dev_info(cm->dev, "%s :VBATT dropped %duV after full-batt(recharge vbatt =%d), batt_ocv(%d)\n",
			__func__, diff, desc->fullbatt_uV - desc->fullbatt_vchkdrop_uV, batt_ocv);
				      ...
				      int jeita_status)
{
	...
	bq2560x_charger_set_termina_cur(pinfo, 240);
...
static int cm_manager_jeita_current_monitor(struct charger_manager *cm)
{
	...
	if(aging_status == 1 && cur_jeita_status == STATUS_ABOVE_T3){
		cm_manager_adjust_current(cm, STATUS_ABOVE_T3);
	}
	...
	if (last_jeita_status == -1) {
		...
		if (last_jeita_status >= desc->jeita_tab_size || last_jeita_status <= 0)
			temp_stop_charging_flag = true;
		else
			temp_stop_charging_flag = false;
		goto out;
	}
	...
	if (cur_jeita_status > last_jeita_status) {
		...
		if (++temp_up_trigger > 2) {
			dev_info(cm->dev, "current-last jeita status: %d-%d, temp up current temperature: %d\n",
				  cur_jeita_status, last_jeita_status, desc->temperature);
			is_normal = cm_manager_adjust_current(cm,
							      cur_jeita_status);
			last_jeita_status = cur_jeita_status;
			if (last_jeita_status >= desc->jeita_tab_size)
				temp_stop_charging_flag = true;
			else {
				temp_stop_charging_flag = false;
			}
		...
		if (++temp_down_trigger > 2) {
			dev_info(cm->dev, "current-last jeita status: %d-%d, temp down current temperature: %d\n",
				  cur_jeita_status, last_jeita_status, desc->temperature);
			is_normal = cm_manager_adjust_current(cm,
							      cur_jeita_status);
			last_jeita_status = cur_jeita_status;
			if (last_jeita_status <= 0)
				temp_stop_charging_flag = true;
			else {
				temp_stop_charging_flag = false;
			}
...
static bool _cm_monitor(struct charger_manager *cm)
{
	 ...
	 */
		...
		if (bat_id == 3) {
			dev_info(cm->dev,
				"battery id: others, stop charging\n");
			return true;
		} else if (bat_ovp_flag == true) {
			dev_info(cm->dev,
				"bat_ovp, stop charging\n");
			return true;
		}else if(i_ovp == 3){
			dev_info(cm->dev,
				"vbus ovp, stop charging\n");
			return true;
		}
		cm->emergency_stop = 0;
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_STATUS:
		if (is_charging(cm)) {
			...
			//} else if(notify_code & (CHG_BAT_LOW_TEMP_STATUS | CHG_BAT_HIG_TEMP_STATUS)){
			} else if(temp_stop_charging_flag){
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
			} else {
				val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
			}
	...
	case POWER_SUPPLY_PROP_NOTIFY_CODE:
		...
		break;
	case POWER_SUPPLY_PROP_AGING_STATUS:
		val->intval = aging_status;
		dev_info(cm->dev, "aging_status:%d\n",aging_status);
		break;
	case POWER_SUPPLY_PROP_SHORT_C_HW_STATUS:
		val->intval = 1;
		dev_info(cm->dev, "short_c_hw_status:%d\n",val->intval);
		break;
...
static enum power_supply_property default_charger_props[] = {
	...
	POWER_SUPPLY_PROP_AGING_STATUS,
...
static void enter_ship_mode_function(void)
{
	int ret = 0;
	if(shipmode == 1) {
		printk("enter shipmode\n");
		ret =bq2560x_set_ship_mode(pinfo);
	} else
		printk("shutdown not enable shipmode in factorytest\n");
}
...
static void cm_bat_temp_notify_check(struct charger_manager *cm)
{
	struct charger_desc *desc = cm->desc;
	int ret, cur_temp, i = desc->jeita_tab_size - 1;
	...
	if (cur_temp < NTC_DISCONNECT_TEMP) {
		...
		cm_uevent_notify(cm);
		dev_err(cm->dev, "%s: bat_temp(%d < %d), battery id unknown!!\n",
				__func__, cur_temp, NTC_DISCONNECT_TEMP);
	} else if (cur_temp < desc->jeita_tab[0].temp) {
		notify_code |= CHG_BAT_LOW_TEMP_STATUS;
		cm_uevent_notify(cm);
		dev_err(cm->dev, "%s: bat_temp(%d < %d), out of range(too low)!!\n",
				__func__, cur_temp, desc->jeita_tab[0].temp);
	} else {
		notify_code &= ~CHG_BAT_LOW_TEMP_STATUS;
	}

	if (cur_temp > desc->jeita_tab[i].temp) {
		notify_code |= CHG_BAT_HIG_TEMP_STATUS;
		cm_uevent_notify(cm);
		dev_err(cm->dev, "%s: bat_temp(%d > %d), out of range(too hig)!!\n",
				__func__, cur_temp, desc->jeita_tab[i].temp);
	} else {
		notify_code &= ~CHG_BAT_HIG_TEMP_STATUS;
	}

	//dev_err(cm->dev, "%s: low_temp: %d, hig_temp: %d!!\n", __func__, desc->jeita_tab[0].temp, desc->jeita_tab[i].temp);
}
...
static void cm_bat_ovp_check(struct charger_manager *cm)
{
	...
	if (batt_uV >= BATTERY_VOLTAGE_MAX) {
		...
		//cm->emergency_stop = CHG_BAT_OV_STATUS;
		//try_charger_enable(cm, false);
		charger_dev_stop_charge(pinfo);
		cm_uevent_notify(cm);
		power_supply_changed(cm->charger_psy);
...
{
	int val = 0, ret = 0;
	ret = iio_read_channel_processed(cm->desc->channel, &val);
	if (ret < 0) {
		dev_err(cm->dev, "bat_id IIO channel read failed %d\n", ret);
		return;
	}

	if ((val >= ATL_BATTERY_VOLTAGE_MIN_5000)&&(val <= ATL_BATTERY_VOLTAGE_MAX_5000)) {
		bat_id = 1;
		get_hardware_info_data(HWID_BATERY_ID,"ATL");
		dev_err(cm->dev, "battery id:ATL id_volt:%d \n", val);
		notify_code &= ~CHG_BAT_ID_STATUS;
		//charger_dev_start_charge(pinfo);
		//power_supply_changed(cm->charger_psy);
		//cm->charging_status &= ~CM_CHARGE_BATID_ABNORMAL;
	} else if((val >= LIWINON_BATTERY_VOLTAGE_MIN_5000)&&(val <= LIWINON_BATTERY_VOLTAGE_MAX_5000)){
		bat_id = 2;
		get_hardware_info_data(HWID_BATERY_ID,"LIWINON");
		dev_err(cm->dev, "battery id:LIWINON id_volt:%d \n", val);
		notify_code &= ~CHG_BAT_ID_STATUS;
	}else{
		bat_id = 3;
		get_hardware_info_data(HWID_BATERY_ID,"Unknown");
		dev_err(cm->dev, "battery id:others id_volt:%d \n", val);
...
#define EMERGENCY_TEMP 850  //added by zrs for 85
...
static void cm_batt_works(struct work_struct *work)
{
	...
	int period_time, flush_time,  board_temp;
	int cur_temp;
	...
	cm_bat_id_check(cm);
	cm_bat_ovp_check(cm);
	cm_bat_temp_notify_check(cm);

	//dev_info(cm->dev, "charger volt is:%d\n",charger_input_vol);
	if(charger_input_vol >= CHARGER_HV_THR && i_ovp != 3){
		dev_err(cm->dev, "charger volt is > 5800000uv:%d\n",charger_input_vol);
		i_ovp++;
		if(i_ovp > 2){
			i_ovp = 3;
			vbus_state = 1;
			notify_code |= CHG_VBUS_OV_STATUS;
			//bq2560x_charger_stop_charge(pinfo);
			cm->emergency_stop = 8;
			charger_dev_stop_charge(pinfo);
			power_supply_changed(cm->charger_psy);
			cm_uevent_notify(cm);
			dev_info(cm->dev, "charger stop:i_ovp:%d,vbus_state:%d,notify_code:0x%x\n",i_ovp,vbus_state,notify_code);
		}
	} else if(charger_input_vol < CHARGER_HV_THR){
		if(vbus_state != 0){
			i_ovp = 0;
			vbus_state = 0;
			notify_code &= ~CHG_VBUS_OV_STATUS;
			//bq2560x_charger_start_charge(pinfo);
			charger_dev_start_charge(pinfo);
			power_supply_changed(cm->charger_psy);
			dev_info(cm->dev, "charger stop:i_ovp:%d,vbus_state:%d,notify_code:0x%x\n",i_ovp,vbus_state,notify_code);
		}
...
//#endif
	...
	printk("charger-manager: battery voltage = %d, OCV = %d, current = %d, "
		 ...
		 "is_fast_charge = %d, enable_fast_charge = %d, timeout_flag = %d, temp_stop = %d\n",
		 batt_uV, batt_ocV, bat_uA, fuel_cap, ui_cap, cm->desc->charger_status,
		 ...
		 cm->desc->enable_fast_charge, timeout_flag, temp_stop_charging_flag);

	bq2560x_disable_safety_timer(pinfo);
	bq2560x_dump_regs(pinfo);
	...
	if (cur_temp >=EMERGENCY_TEMP){
			printk("cur_temp > emergency_temp\n");
			set_batt_cap(cm, 0);
		printk("set_batt_cap\n");
		kernel_power_off();
		printk("kernel_power_off\n");
			orderly_poweroff(true);
		printk("orderly_poweroff\n");
		} //added by zrs for power off (temp>85)
/*	if (cur_temp >= 530){
		printk("cur_temp > 530\n");
		charger_dev_disable_charge(pinfo,true);
		printk("more_than_temp53 charger_dev_stop_charge");
			} //added by zrs for stopping charge  (temp>53)*/
	if(bat_id == 2){
		cm->desc->shutdown_voltage = 3400000;
	}

     if(cur_temp >= 450 && bat_uA < 260000 && batt_uV > WARM_FULLBAT_VOLTAGE){
			printk("CHG_BAT_TEMP_HIG_STOP_STATUS1\n");
			notify_code |= CHG_BAT_TEMP_HIG_STOP_STATUS;
			//bq2560x_charger_stop_charge(pinfo);
			power_supply_changed(cm->charger_psy);
			cm_uevent_notify(cm);
			printk("CHG_BAT_TEMP_HIG_STOP_STATUS2= 0x%x\n,",notify_code);
		}else {
			printk("CHG_BAT_TEMP_HIG_STOP_STATUS3\n");
			notify_code &= ~CHG_BAT_TEMP_HIG_STOP_STATUS;
			power_supply_changed(cm->charger_psy);
            printk("CHG_BAT_TEMP_HIG_STOP_STATUS4= 0x%x\n,",notify_code);
		}

	if (batt_uV <= cm->desc->shutdown_voltage && (cm->desc->charger_status == POWER_SUPPLY_STATUS_NOT_CHARGING ||
		...
		if (shutdown_low_cap_count >= 3) {
			...
			dev_err(cm->dev, "WARN: batt_uV less than 3.4, adjust_fuel_cap(cm, 0)\n");
	...
	if (batt_uV <= cm->desc->shutdown_voltage - CM_UVLO_OFFSET) {
		...
		/*if (batt_uV <= 3260000) {
			set_batt_cap(cm, 0);
			dev_err(cm->dev, "WARN: batt_uV less than 3260000, will shutdown\n");
			orderly_poweroff(true);
		}*/
	} else {
	...
	dev_info(cm->dev, "battery cap = %d, charger manager cap = %d, ui_cap = %d\n",
		 fuel_cap, cm->desc->cap, DIV_ROUND_CLOSEST(fuel_cap, 10));
...
static int charger_manager_probe(struct platform_device *pdev)
{
	...
	desc->channel = iio_channel_get(&pdev->dev, "batt_id-channel");
	if (IS_ERR(desc->channel)) {
		ret = PTR_ERR(desc->channel);
		dev_err(cm->dev, "IIO channel not found: %d\n", ret);
		//return ret;
	}
