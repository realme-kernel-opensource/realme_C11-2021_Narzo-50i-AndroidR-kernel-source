...
static int get_battery_id(void)
{
	...
	if(match){
		...
		if(result[0] == '1'){
			printk("battery_id:ATL");
			return 1;
		}else if (result[0] == '2'){
			printk("battery_id:LIWINON");
			return 2;
		}else{
			printk("battery_id:ELSE");
			return 3;
		}
	}
	return 0;
				  ...
				  struct power_supply_battery_info *info, int num)
{
	...
	info->bat_id = get_battery_id();
	printk("bat_id:%d",info->bat_id);

	if(info->bat_id == 2){
		num = 1;
	}

	printk("%s bat_num:%d",__func__,num);
	...
	for (index = 0; index < len; index++) {
		...
		for (i = 0; i < tab_len; i++) {
			...
			printk("zrstable[i].ocv = %d",table[i].ocv );
			table[i].capacity = be32_to_cpu(*list++);
			printk("zrstable[i].capacity = %d",table[i].capacity );
		}
	}
