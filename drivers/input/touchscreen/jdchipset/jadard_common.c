...
struct proc_dir_entry *JAD_proc_touchpanel_dir = NULL;
struct proc_dir_entry *JAD_proc_baseline_test_file = NULL;
struct proc_dir_entry *JAD_proc_backscreen_baseline_file = NULL;
...
void jadard_usb_mode_switch(int val)
{
	if (val) {
		jd_usb_detect_flag = true;
	} else {
		jd_usb_detect_flag = false;
	}
	JD_I("%s: jd_usb_detect_flag = 0x%2.2X\n", __func__, jd_usb_detect_flag);
}
#endif
        ...
        struct device_attribute *attr, char *buf)
{
    ...
    return sprintf(buf, "ts->suspended=%d\n", ts->suspended);
}
									...
									size_t len, loff_t *pos)
{
	...
	if (!pjadard_debug->proc_send_flag) {
		...
		if (buf_tmp != NULL) {
			...
			if (val == 0) {
				ret += snprintf(buf_tmp + ret, len - ret, "passed\n", val);
			} else {
				ret += snprintf(buf_tmp + ret, len - ret, "Sorting_Test failed\n", val);
			}
...
static ssize_t jadard_sorting_black_test_read(struct file *file, char *buf,
									size_t len, loff_t *pos)
{
	size_t ret = 0;
	char *buf_tmp = NULL;
...
static struct file_operations jadard_proc_black_test_ops = {
	.owner = THIS_MODULE,
	.read = jadard_sorting_black_test_read,
};
...
#ifdef JD_ROTATE_BORDER
void jadard_rotate_mode_switch(char *buf, int count)
{
	struct jadard_ts_data *ts = pjadard_ts_data;
	char buf_tmp[4] = {0};
	int RotateType;
	...
	if ((RotateType < 0) || (RotateType > 2)) {
		JD_E("%s: RotateType must (0,1,2), BUF: %s\n", __func__, buf);
		return;
	}
...
#ifdef JD_EARPHONE_DETECT
void jadard_earphone_mode_switch(int val)
{
	struct jadard_ts_data *ts = pjadard_ts_data;
...
int jadard_common_proc_init(void)
{
	...
	JAD_proc_touchpanel_dir = proc_mkdir(JADARD_PROC_TOUCHPANLE_FOLDER, NULL);
	...
	JAD_proc_baseline_test_file = proc_create(OEM_SELFTEST_PROC_FILENAME, (S_IRUGO),
										JAD_proc_touchpanel_dir, &jadard_proc_baseline_test_ops);
	if (JAD_proc_baseline_test_file == NULL) {
		JD_E(" %s: proc self_test file create failed!\n", __func__);
		goto fail_1;
	}

	JAD_proc_backscreen_baseline_file = proc_create(OEM_SELFTEST_BLACK_FILENAME, (S_IRUGO),
										JAD_proc_touchpanel_dir, &jadard_proc_black_test_ops);
	if (JAD_proc_backscreen_baseline_file == NULL) {
...
fail_2:
    ...
    remove_proc_entry(OEM_SELFTEST_PROC_FILENAME, JAD_proc_touchpanel_dir);
	remove_proc_entry(OEM_SELFTEST_BLACK_FILENAME, JAD_proc_touchpanel_dir);
...
int jadard_chip_common_init(void)
{
	...
	tp_interface.charger_mode_switch_status = jadard_usb_mode_switch;
	tp_interface.headset_switch_status = jadard_earphone_mode_switch;
	tp_interface.tp_inferface_edge_mode =  jadard_rotate_mode_switch;
