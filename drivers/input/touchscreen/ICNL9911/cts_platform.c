        ...
        struct device_node *dev_node)
{
   ...
   ret = of_property_read_u32(dev_node, CFG_CTS_OF_XY_RESOLUTION_NAME,coords);
   pdata->res_x = 720;
   pdata->res_y = 1600;
  // printk(" pdata->res_y =%d  pdata->res_x=%d \n",__func__,__LINE__,pdata->res_y,pdata->res_x);
...
/*
    ret = of_property_read_u32(dev_node, CFG_CTS_OF_XY_RESOLUTION_NAME,
            &pdata->res_x);
    if (ret) {
        cts_warn("Parse X resolution from dt failed %d", ret);
		pdata->res_x = 720;
