...
void cts_edge_mode_set(char *buf,int cnt)
{
    ...
    printk("%s %d buf = %c \n",__func__,__LINE__,buf[0]);
		...
		if (buf[0] == '1')	{
			printk(" %s %d  left \n",__func__,__LINE__);
			cts_fw_reg_writeb_retry(cts_dev, 0x801, 1, 3, 0);//横屏，刘海在左
		}
		else if (buf[0] == '2')  {
			printk(" %s %d  right \n",__func__,__LINE__);
			cts_fw_reg_writeb_retry(cts_dev, 0x801, 2, 3, 0);//横屏，刘海在右
		}
		else if (buf[0] == '0') {
			printk(" %s %d  null \n",__func__,__LINE__);
			cts_fw_reg_writeb_retry(cts_dev, 0x801, 0, 3, 0);//竖屏	
		}
