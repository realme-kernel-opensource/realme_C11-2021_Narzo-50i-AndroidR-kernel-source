...
#define VENDOR_WIFI_CALI_FILE_RIOE "/vendor/etc/connectivity_calibration_rioe.ini"
...
char *get_project_name()
{
	...
	if(ptr != NULL){
		...
		printk("[kernel] androidboot.product.prjversion: %s", ptr);
	}
...
int get_connectivity_cali_param(struct wifi_cali_t *p)
{
	...
	if (ret < 0) {
		...
		if (IS_ERR(file)) {
			pr_err("open file %s error,try vendor/etc\n", path);
			if ((prj_name != NULL) && (strncmp(prj_name, "20782", 5) == 0 ))
			{
				path = VENDOR_WIFI_CALI_FILE_RIOE;
			} else {
