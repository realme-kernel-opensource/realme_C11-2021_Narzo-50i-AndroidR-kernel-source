...
#define HARDWARE_INFO_VERSION   "realme C11 2021"
#define HARDWARE_INFO_VERSION_2 "narzo 50i"
#define HARDWARE_INFO_WCN       "realme C11 2021"
#define HARDWARE_INFO_WCN_2     "narzo 50i"
...
*/
#define EMMC_VENDOR_NAME "/sys/devices/platform/soc/soc:ap-ahb/20600000.sdio/mmc_host/mmc0/mmc0:0001/name"
...
#define EMMC_VENDOR_VERSION "/sys/devices/platform/soc/soc:ap-ahb/20600000.sdio/mmc_host/mmc0/mmc0:0001/fwrev"
...
static ssize_t show_flash(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	return sprintf(buf, "flash name :%s %dGB+%dGB %s", DDR_TYPE, get_ram_size(), get_emmc_size(), emmc_vendor_name);
}
...
static ssize_t show_main_cam_sn(struct device *dev, struct device_attribute *attr, char *buf)
{
    if (hwinfo_data.main_cam_sn != NULL)
        return sprintf(buf, "MAIN Cam_SN :%s\n", hwinfo_data.main_cam_sn);
    else
        return sprintf(buf, "MAIN Cam_SN :Not_found\n");
}
...
static int hw_info_parse_dt(struct device_node *np)
{
    ...
    if (np) {
        ...
        if (gpio_sdcard < 0) {
                printk("[HWINFO] sdcard-gpio not available (ret=%d)\n", ret);
                gpio_sdcard = 473;
        }
