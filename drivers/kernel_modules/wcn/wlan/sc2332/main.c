...
#define CMD_SET_FCC_EN			"SET_FCC_EN"
...
static int sprdwl_set_power_save(struct net_device *ndev, struct ifreq *ifr)
{
	...
	} else if (!strncasecmp(command, CMD_SET_FCC_EN,
				strlen(CMD_SET_FCC_EN))) {
		skip = strlen(CMD_SET_FCC_EN) + 1;
		ret = kstrtoint(command + skip, 0, &value);
		if (ret)
			goto out;
		netdev_info(ndev, "%s: set fcc test en,value : %d\n",
			    __func__, value);
		ret = sprdwl_power_save(priv, vif->mode,
					SPRDWL_SET_FCC_EN, value);
	} else {
		netdev_err(ndev, "%s command not support\n", __func__);
		ret = -ENOTSUPP;
	}
