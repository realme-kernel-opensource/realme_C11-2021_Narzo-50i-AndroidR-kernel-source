...
#define MAX_COUNTRY_COUNT	9
const char fcc_country[MAX_COUNTRY_COUNT][2] = {"UY", "CL", "PE", "EC", "MX", "CO", "BR", "BO", "TW"};
			      ...
			      struct regulatory_request *request)
{
	...
	/*check country code for fcc channel*/
	for (i = 0; i < MAX_COUNTRY_COUNT; i++) {
		if (fcc_country[i][0] == request->alpha2[0] &&
		    fcc_country[i][1] == request->alpha2[1]) {
			wiphy_err(wiphy,"need enable fcc: %c%c\n",
				  fcc_country[i][0], fcc_country[i][1]);
			fcc_enable = 1;
			break;
		    }
	}

	wiphy_info(wiphy, "fcc enable value : %d\n", fcc_enable);
	ret = sprdwl_power_save(priv, SPRDWL_MODE_NONE,SPRDWL_SET_FCC_EN, fcc_enable);
	if (ret)
		wiphy_err(wiphy,"set fcc failed : %d\n", fcc_enable);
}
