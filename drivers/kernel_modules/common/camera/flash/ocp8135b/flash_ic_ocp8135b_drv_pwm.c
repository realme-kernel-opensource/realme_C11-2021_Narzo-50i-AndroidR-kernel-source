...
static int sprd_pin62_pwm_selected(void *drvd, bool is_pwm_selected)
{
    struct flash_driver_data *drv_data = (struct flash_driver_data *)drvd;
    int ret;

    if (is_pwm_selected) {
        ret =  pinctrl_select_state(drv_data->pin_gpio_62, drv_data->pinctrl_state_pwm);
    } else {
        ret =  pinctrl_select_state(drv_data->pin_gpio_62, drv_data->pinctrl_state_gpio);
    }
...
static int sprd_flash_ocp8135b_open_torch(void *drvd, uint8_t idx)
{
    ...
    if (SPRD_FLASH_LED0 & idx) {
        ...
        if(drv_data->is_old_board){
            ...
            if (gpio_is_valid(drv_data->gpio_62_ENM)){
                gpio_direction_output(drv_data->gpio_62_ENM, SPRD_FLASH_ON);
                udelay(5 * 1000);
                gpio_direction_output(drv_data->gpio_62_ENM, SPRD_FLASH_OFF);
            }
            sprd_pin62_pwm_selected(drv_data, true);
...
static int sprd_flash_ocp8135b_close_torch(void *drvd, uint8_t idx)
{
    ...
    if (SPRD_FLASH_LED0 & idx) {
        ...
        pwm_set_ocp8135b_config(drv_data->pwm_chip, 0);
        sprd_pin62_pwm_selected(drv_data, false);
        if (gpio_is_valid(drv_data->gpio_62_ENM)){
            gpio_direction_output(drv_data->gpio_62_ENM, SPRD_FLASH_OFF);
        }
        udelay(5 * 1000);
...
static int sprd_flash_ocp8135b_probe(struct platform_device *pdev)
{
    ...
    drv_data->gpio_62_ENM = of_get_named_gpio(pdev->dev.of_node, FLASH_GPIO_ENM_NAME, 0);
    if (gpio_is_valid(drv_data->gpio_62_ENM)) {
        ret = devm_gpio_request(&pdev->dev,
                    drv_data->gpio_62_ENM, FLASH_GPIO_ENM_NAME);
        pr_err("gpio init gpio %d\n",drv_data->gpio_62_ENM);
        if (ret) {
            pr_err("flash gpio err\n");
            goto exit;
        }
    ...
    ret = gpio_direction_output(drv_data->gpio_62_ENM, SPRD_FLASH_OFF);
    if (ret) {
        pr_err("flash gpio output err\n");
        goto exit;
    }

    drv_data->pin_gpio_62 = devm_pinctrl_get(&pdev->dev);
    drv_data->pinctrl_state_gpio = pinctrl_lookup_state(drv_data->pin_gpio_62, "gpio_62");
    drv_data->pinctrl_state_pwm = pinctrl_lookup_state(drv_data->pin_gpio_62, "gpio_pwm");
