...
int vdsp_log_alloc_buffer(struct xvp *xvp)
{
	...
	pr_debug("log buffer pa:%lx,va:%lx, iova: %lx, size %ld\n",
			ion_buf->addr_p[0] , ion_buf->addr_k[0], ion_buf->iova[0], ion_buf->size[0]);
...
int vdsp_log_init(struct xvp *xvp)
{
	...
	log->flag[1] = 0;
	log->addr[0] = xvp->ion_vdsp_log.addr_p[0] + sizeof(struct log_header_stru);
	log->addr[1] = xvp->ion_vdsp_log.addr_p[0] + sizeof(struct log_header_stru) + log->bank_size;
	log->log_size[0] = 0;	//vdsp write log size
	log->log_size[1] = 0;
	log->log_level = 5;		//vdsp log level
	log->coredump_flag = COREDUMP_NONE;

	pr_info("log header addr:%lx, bank addr:%x,%x\n", xvp->ion_vdsp_log.addr_p[0], log->addr[0], log->addr[1]);
