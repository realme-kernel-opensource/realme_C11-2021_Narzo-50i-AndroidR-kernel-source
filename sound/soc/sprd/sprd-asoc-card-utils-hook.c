...
static int det_type = -1;
static int det_type1 = -1;
...
int hook_gpio_pulse_control_FS1910N(unsigned int gpio,int mode)
{
	unsigned long flags;
	...
	do {
		...
		while (count > 0) { // count of pulse
			gpio_set_value( gpio, EN_LEVEL);
			udelay(2); // < 140us 10-150
			gpio_set_value( gpio, !EN_LEVEL);
			udelay(2); // < 140us
			count--;
		}

		udelay(100); // 50us~190us 40-200
		...
		while (count > 0) { // count of pulse
			gpio_set_value( gpio, EN_LEVEL);
			udelay(20); // < 140us 10-150
			gpio_set_value( gpio, !EN_LEVEL);
			udelay(20); // < 140us
			count--;
		}

		udelay(700); // 500us~2ms 520-2000
		// 3. pull up gpio and delay, enable pa
		gpio_set_value( gpio, EN_LEVEL);
		// check_intervel_time(spc19xx, 500, 1950);
		spin_unlock_irqrestore(lock, flags);

		udelay(400); // pull up gpio > 220us
		//spc19xx->spc_mode = mode;
		break;
	} while (0);
...
static int hook_general_spk(int id, int on)
{
    ...
    if(!det_type1){
        pr_info("%s det_type1: %d, siaxxx\n",__func__, det_type1);
		hook_gpio_pulse_control(gpio, mode);
	} else if(det_type){
        pr_info("%s det_type: %d, awxxx\n",__func__, det_type);
		hook_gpio_pulse_control(gpio, mode);
	} else{
        pr_info("%s det_type: %d, fs19xx\n",__func__, det_type);
		hook_gpio_pulse_control_FS1910N(gpio, mode);
	}
					 ...
					 struct sprd_asoc_ext_hook *ext_hook)
{
	...
	const char *prop_pa_det1 = "sprd,spk-ext-pa-det1";
    int det_gpio;
	int det_gpio1;
	...
	for (i = 0; i < spk_cnt; i++) {
		...
		// det
		det_gpio1 = of_get_named_gpio_flags(np, prop_pa_det1, i, NULL);
		if (det_gpio1 < 0) {
			dev_err(dev, "%s not exit! skip\n", prop_pa_det1);
		} else {
			// detect pa type
			gpio_flag = GPIOF_DIR_IN;
			ret = gpio_request_one(det_gpio1, gpio_flag, "det_gpio1");
			if (ret < 0) {
				dev_err(dev, "det_gpio1 det request failed:%d!\n", ret);
				return ret;
			}
			det_type1 = gpio_get_value(det_gpio1);
			dev_info(dev, "det_gpio1 det:%d\n", det_type1);
		}
	}
	return 0;
}
